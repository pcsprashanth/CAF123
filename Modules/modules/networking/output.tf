...
output "private_endpoint_vnets" {
  description = ""
  value       = distinct([for s in azurerm_subnet.this : split("/subnet", s.id)[0] if(s.private_endpoint_network_policies_enabled != false && s.private_link_service_network_policies_enabled != false)])
}
...
output "private_endpoint_subnets" {
  description = ""
  value       = distinct([for s in azurerm_subnet.this : s.id if(s.private_endpoint_network_policies_enabled != false && s.private_link_service_network_policies_enabled != false)])
}
